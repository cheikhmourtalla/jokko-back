...
exports.getStockAlerts = exports.streamNotifications = void 0;
exports.sendNotificationToShop = sendNotificationToShop;
...
function sendNotificationToShop(shopId, event, data) {
...
const streamNotifications = async (req, res) => {
    ...
    logger_1.logger.info(`📡 SSE connecté — Shop: ${shopId} — User: ${userId} — Clients actifs: ${clients.get(shopId).size}`);
    ...
    try {
        ...
        const lowStock = allProducts.filter((p) => p.quantity > 0 && p.quantity <= p.alertThreshold);
        ...
        logger_1.logger.error("Erreur envoi alertes initiales SSE", { error });
    }
    ...
    req.on("close", () => {
        ...
        logger_1.logger.info(`📡 SSE déconnecté — Shop: ${shopId} — User: ${userId}`);
    });
};
...
const getStockAlerts = async (req, res) => {
    try {
        ...
        const lowStock = products.filter((p) => p.quantity > 0 && p.quantity <= p.alertThreshold);
